...
export default function Terminos() {
  ...
  return (
    <main className="page">
      <title>{t('terminos.title')} — CicloIT</title>
      <meta name="description" content={t('terminos.lede')} />
      <meta property="og:title" content={`${t('terminos.title')} — CicloIT`} />
      <meta property="og:url" content="https://cicloit.com/terminos" />
      <meta property="og:type" content="website" />
      <link rel="canonical" href="https://cicloit.com/terminos" />
      <section className="section" style={{ paddingTop: 'calc(var(--pad-section) * .55)' }}>
        <div className="container legal-doc">
          <Eyebrow>{t('terminos.eyebrow')}</Eyebrow>
          <h1 className="h-display" style={{ marginTop: 16 }}>{t('terminos.title')}</h1>
          <p className="lede" style={{ marginTop: 20 }}>{t('terminos.lede')}</p>

          <div className="legal-body">
            {sections.map(s => (
              <div key={s} className="legal-section">
                <h2 className="h-3">{t(`terminos.${s}.title`)}</h2>
                <p className="muted">{t(`terminos.${s}.body`)}</p>
              </div>
            ))}
          </div>
